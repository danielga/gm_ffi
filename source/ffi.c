...
#define TO_NUMBER(TYPE, ALLOW_POINTERS)                                     \
    ...
    case LUA_TUSERDATA:                                                     \
        ...
        if (ct.type == INVALID_TYPE) {                                      \
            ...
            real = (TYPE) check_intptr(L, idx, p, &ct);                            \
        }                                                                   \
        lua_pop(L, 1);                                                      \
        break;                                                              \
                                                                            \
...
{ double ret; do_check_double(L, idx, &ret, NULL); return (float) ret; }
...
static ptrdiff_t lookup_cdata_index(lua_State* L, int idx, int ct_usr, struct ctype* ct)
{
    ...
    case LUA_TNUMBER:
        ...
        return (ct->pointers ? sizeof(void*) : ct->base_size) * (ptrdiff_t) lua_tonumber(L, 2);
...
#define POWC(l,r,s) s = (int64_t) cpow((double) l, (double) r)
#else
...
static int ffi_number(lua_State* L)
{
    ...
    if (ct.type != INVALID_TYPE) {
        lua_pushnumber(L, (lua_Number) check_intptr(L, 1, data, &ct));
        return 1;
    } else {
